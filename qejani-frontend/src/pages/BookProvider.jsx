...
const socket = io('http://localhost:5001') // adjust
...
export default function BookProvider() {
  ...
  useEffect(() => {
    const fetchProvider = async () => {
      try {
        const res = await fetch(`http://localhost:5001/api/providers/${providerId}`, {
  ...
  const handlePay = async () => {
    ...
    setPayLoading(true)
    try {
      const res = await fetch(`http://localhost:5001/api/mpesa/stkpush?providerId=${providerId}`, {
        method: 'POST',
        headers: {
  ...
  const handleFinishService = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5001/api/mpesa/finish-payment', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4">
      <div className="bg-white rounded-3xl shadow-xl p-6 sm:p-10 max-w-lg w-full border border-gray-100">
